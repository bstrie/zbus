...
use crate::{BusName, EndianSig, Error, InterfaceName, MIN_MESSAGE_SIZE, MessageField, MessageFieldCode, MessageFields, MessageHeader, MessagePrimaryHeader, MessageType, NATIVE_ENDIAN_SIG, PRIMARY_HEADER_SIZE, Result, UniqueName, owned_fd::OwnedFd, utils::padding_for_8_bytes};
...
impl Message {
    ...
    pub fn method<'s, 'd, 'p, 'i, B, SE, DE, PE, IE>(
    ...
    pub fn signal<'s, 'd, 'p, 'i, B, SE, DE, PE, IE>(
        sender: Option<impl TryInto<UniqueName<'s>, Error = SE>>,
        destination: Option<impl TryInto<BusName<'d>, Error = DE>>,
        path: impl TryInto<ObjectPath<'p>, Error = PE>,
